...
public class GoblinExplore : GoblinState {
    ...
    public override void Act() {
        ...
        if(!gsc.IsWaiting()){
            ...
            if(Vector3.Distance(goblin.transform.position, destination) > 0.5f) {
                goblin.transform.position = Vector3.MoveTowards(goblin.transform.position, destination, 0.02f);
            } else {
                NewDestination();
            }
    ...
    public override void OnStateEnter() {
        ...
        currentRoom = gsc.GetCurrentRoom().transform;
        NewDestination();
        gsc.StartExploring();
    }
    ...
    private void NewDestination() {
        ...
        goblin.transform.LookAt(destination);
    }
}
